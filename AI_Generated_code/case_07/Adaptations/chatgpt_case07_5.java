...
public class MatrixDiagonalSumRecursive {
    ...
    public static double sumMajorDiagonal(double[][] m, int index) {
        // Base case: When index reaches the size of the matrix, stop recursion
        if (index == m.length) {
            return 0;
        }
        // Recursive call to sum the next diagonal element
        return m[index][index] + sumMajorDiagonal(m, index + 1);
    }
